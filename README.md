...
カメラか画像ファイルを渡すと、その場で読みます。**認識器は自前で学習したモデル**
（`models/plate_ocr_v2.onnx`、実データ hold-out の地域名 **97.9%**）。検出器はページ上で切り替えられます:

| 検出器 | 状態 |
|---|---|
| PlateYOLO-JP 320（借り物・AGPL） | 実写に強いが**車ごと写す前提**（プレートが画面幅の 5–12% が最良帯、30% 超は検出ゼロ） |
| 自前 yolov8n 320 | 学習中。合成データだけなので実写ではまだ弱い（近接は取れる方向） |
