...
（`models/plate_det_v8n_320.onnx` + `models/plate_ocr_v4.onnx`、実データ hold-out の地域名 **97.9%**）。
...
| 認識器（地名を138名すべて言えるか） | 15.9% → **50.4%** | `tools/check_regions.py`（地名1つずつレンダして読ませる）。v2 → v5。2025 追加の 5 名も 4/5 が読めるようになった（安曇野 2/2、江戸川・日光・南信州 1/2、十勝 0/2） |
