...
確定します。1 フレームは既定の 640 で 5.4 秒、320 に落とせば 0.44 秒。推論は Web Worker）。**検出器も認識器も自前で学習したモデル**
（既定は `models/plate_det_v8n_640.onnx` + `models/plate_ocr_v7_bal.onnx`、実データ hold-out の地域名 **97.9%**）。
...
|---|---|---|---|
| **自前 yolov8n 640**（既定） | **近接も遠景も**（画面 11% のバスで 0.41、近接の黒ナンバーで 0.93） | 検出だけで 4 倍重い | **97.1%** |
| 自前 yolov8n 320 | **プレート単体・近接**（実写の黒ナンバーで det 0.93、車体が写っていなくても取れる）、4 倍軽い | 画面 10% 前後の遠景を落とす | 93.7% |
| PlateYOLO-JP 320（借り物・AGPL・比較用） | 車ごと写った遠景（同じバスで **0.80**） | **近接は検出ゼロ**（20% で 0.07、30% 超はゼロ） | 13.7%（このセットは車体なしが主なので不利） |
...
手元の実写をそのまま入れた結果（`tools/infer.py --out` が描いた画像）。
